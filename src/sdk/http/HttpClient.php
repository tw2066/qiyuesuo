...
class HttpClient{
    function doService($url,HttpParameter $httpParameters, $headers, $connectTimeout, $readTimeout){
        switch ($httpParameters->getHttpMethod()) {
            case HttpMethod::GET:
                $result = $this->doGet($url,$headers,$httpParameters->getParams(), $connectTimeout, $readTimeout);
                break;
            case HttpMethod::POST:
                $result = $this->doPost($url,$headers,$httpParameters->getParams(), $connectTimeout, $readTimeout);
                break;
            default:
                $result = null;
                break;
    ...
    function doDownload($url,HttpParameter $httpParameters, $headers, $connectTimeout, $readTimeout, $filePath){
        $result = $this->doGet($url,$headers,$httpParameters->getParams());
        //判断是否返回文件流
