...
class SDKClient{
    ...
    public function service(SdkRequest $baseRequest) {
        ...
        if($httpParamers->isJson()){
            $result = (new HttpClient())->doServiceWithJson($url, $httpParamers->getJsonParams(), $httpHeader, self::connectTimeout, self::readTimeout);
        } else {
    ...
    public function downloadService(SdkRequest $baseRequest, $filePath) {
        ...
        $httpHeader = $httpHeader->getArray();
        $httpParamers = $baseRequest->getHttpParamers();
        return (new HttpClient())->doDownload($url, $httpParamers, $httpHeader, self::connectTimeout, self::readTimeout, $filePath);
    }
